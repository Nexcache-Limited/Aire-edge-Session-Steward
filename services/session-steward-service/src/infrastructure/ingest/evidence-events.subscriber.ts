...
@Injectable()
export class EvidenceEventsSubscriber implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(EvidenceEventsSubscriber.name);
  ...
  constructor(
    ...
    private readonly normalizer: EvidenceEventNormalizer,
    private readonly events: SessionEventsService,
  ) {}

  onModuleInit(): void {
    if (!this.nats.connection) return;
    const subject = this.config.get<string>('NATS_EVIDENCE_SUBJECT', 'aire.*.evidence.>');
  ...
  private async consume(subscription: Subscription): Promise<void> {
    for await (const message of subscription) {
      try {
        ...
        this.logger.warn(
          `Rejected evidence event: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
  }
